...
#  Choices: any of ['sha3_384', 'sha1', 'blake2s', 'sha256', 'sha512', 'sha3_256', 'md5', 'sha224', 'sha3_512', 'sha384', 'sha3_224', 'blake2b']
#  Default: 'sha256'
...
async def post_spawn_hook(spawner):
    ...
    if auth_state and "fabric_roles" in auth_state:
        home_dir = spawner.home_dir  # resolves to the user's actual home dir
        roles_path = os.path.join(home_dir, ".fabric", "roles.json")
        ...
        try:
            ...
            spawner.log.warning(f"Failed to write fabric_roles: {e}")
